...
function index_url($url, $level, $site_id, $md5sum, $domain, $indexdate, $sessid, $can_leave_domain, $reindex) {
	...
	if ($url_status['state'] == 'ok') {
		...
		if (($md5sum != $newmd5sum || $reindex ==1) && $OKtoIndex == 1) {
			...
			$cws->set_rule('/usr/local/scws/etc/rules.utf8.ini'); //注意路径
			...
			//$wordarray = unique_array(explode(" ", $data['content']));		//原有英文分词
			...
			if ($OKtoIndex == 1) {
				...
				//$wordarray = calc_weights ($wordarray, $title, $host, $path, $data['keywords']);	//删除原有英文分词
